...
export default class OnlineDriver extends LocalDriver {
    constructor(parent, peerParent, socket, config) {
        super(parent, config);
    ...
    startOnline = (data) => {
        ...
        // Set peer config
        this.peerConfig = data.peerConfig;

        // Seed random functions
        this.seeds = data.randSeeds;
        State.setSeed(this.state, this.seeds);
        State.setSeed(this.peerStateAt(0), this.seeds);
    ...
    startPeers = () => {
        this.socket.on("online-event", (data) => {
            ...
            if (data.frame > this.frameIndex)
                console.log(
                    "online-event data received frame later than current frame"
                );
            ...
            for (
                let f = this.recordIndex;
                f < Math.min(this.frameIndex, data.frame - 60);
                f++
            )
                this.peerRecord.shift();
            this.recordIndex = Math.max(
                0,
                Math.min(this.frameIndex, data.frame - 60)
            );
    ...
    reconcile = (frame) => {
        for (let f = frame; f <= this.frameIndex; f++) {
            ...
            if (index + 1 == this.peerRecord.length)
                this.peerRecord.push({ state: undefined, inputs: [] });
            const state = (this.peerRecord[index + 1].state = structuredClone(
                this.peerRecord[index].state
            ));
            ...
            Game.process(
                ...
                state,
                inputs,
                inputs.includes("garbage-accept")
            );
    ...
    onFrame = () => {
        ...
        if (this.state.attack > 0)
            this.peerStateAt(this.frameIndex + 1).garbage.push(
                this.state.attack
            );
        this.state.attack = 0;
        ...
        // Send heartbeat
        if (this.frameIndex % HEARTBEAT_RATE == 0)
            this.publishEvent("heartbeat");
        // DEBUG
        ...
        // Calculate Time.
        const timeError = this.scheduledTickTime
            ? startTime.getTime() - this.scheduledTickTime
            : 0;
        const timeElapsed = new Date().getTime() - startTime.getTime();
        const waitTime = Math.max(
            0,
            1000 / FRAME_RATE - timeElapsed - timeError
        );
        this.scheduledTickTime = new Date().getTime() + waitTime;

        // DEBUG
        if (this.lastFrameTime == 0) this.lastFrameTime = new Date().getTime();
        else
            this.avgDelta =
                (this.avgDelta + (startTime.getTime() - this.lastFrameTime)) /
                (this.avgDelta ? 2 : 1);
        this.avgError = (this.avgError + timeError) / (this.avgError ? 2 : 1);
        this.avgElapsed =
            (this.avgElapsed + timeElapsed) / (this.avgElapsed ? 2 : 1);

        this.lastFrameTime = startTime.getTime();

        console.log(
            "avg Error %d, avg Delta: %d, av Elapsed: %d",
            this.avgError,
            this.avgDelta,
            this.avgElapsed
        );
        ...
        if (
            !this.over &&
            !this.state.over &&
            !this.peerStateAt(this.frameIndex).over
        )
